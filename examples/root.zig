...
test "examples: kv store recovery scenario is replayable" {
    ...
    try std.testing.expectEqualStrings(a, b);
    try std.testing.expect(std.mem.indexOf(u8, a, "disk.fault op=3 path=kv.wal kind=crash_lost_write rate=1/1 roll=0 fired=true") != null);
    try std.testing.expect(std.mem.indexOf(u8, a, "disk.read op=5 path=kv.wal offset=16 len=16 status=corrupt") != null);
...
test "examples: kv store checker catches torn record recovery" {
    ...
    switch (report) {
        ...
        .failed => |failure| {
            ...
            try std.testing.expect(std.mem.indexOf(u8, failure.first_trace, "disk.crash_write op=3 path=kv.wal offset=16 len=16 result=torn") != null);
