...
| High | Fixed (`f875bcd`; suspended read/delete and write/rename regressions) | Concurrent delete/rename can retire `FileState` or replace/free a metadata path while a streaming operation is suspended. Stable `FileMeta` storage does not close this separate handle/path lifetime gap. | file-handle retirement and rename in `src/io/backend.zig`; suspended helpers in `src/io/file.zig` |
