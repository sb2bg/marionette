...
accepted-peer check, send-half-close/EOF scenario, and IPv4 wildcard-listener
exchange against host `std.Io.net` and Marionette. Fault behavior remains an
explicit simulator abstraction rather than an attempt to reproduce one host
kernel.
