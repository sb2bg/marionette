...
pub fn decode(frame: []const u8) FrameError!Decoded {
    ...
    const declared_size = std.mem.readInt(u32, frame[size_offset..][0..size_size], .little);
    if (declared_size < header_len) return error.InvalidSize;
    if (declared_size != frame.len) return error.InvalidSize;

    const reserved = std.mem.readInt(u32, frame[reserved_offset..][0..reserved_size], .little);
    if (reserved != 0) return error.ReservedNonZero;

    const expected_header_checksum = std.mem.readInt(u128, frame[checksum_header_offset..][0..checksum_size], .little);
    if (headerChecksum(frame[0..header_len]) != expected_header_checksum) {
    ...
    return .{
        .from = std.mem.readInt(u16, frame[from_offset..][0..node_id_size], .little),
        .to = std.mem.readInt(u16, frame[to_offset..][0..node_id_size], .little),
        .payload = payload,
    };
}
